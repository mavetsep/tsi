...
def criar_tarefa(id_lista):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'INSERT INTO tarefa (lista_id, descricao_tarefa) VALUES (?, ?)',
                (id_lista, descricao)
            )
            db.commit()
...
def editar_tarefa(id_tarefa):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'UPDATE tarefa SET descricao_tarefa = ?, completada = ? WHERE id = ?',
...
def deletar_tarefa(id_tarefa):
    ...
    db = get_db()
    db.execute('DELETE FROM tarefa WHERE id = ?', (id_tarefa,))
    db.commit()
...
def alternar_status_tarefa(id_tarefa):
    ...
    db = get_db()
    db.execute(
        'UPDATE tarefa SET completada = ? WHERE id = ?',
